...
class _MyStatefulWidgetState3 extends State<MyStatefulWidget3> {
  ...
  void initState() {
    ...
    sampleData
        .add(new RadioModel(sampleData.length - 1, false, Icons.add, 'Ещё'));

    DBMarkerProvider.db.getAllMarks().then((list) {
      for (int i = 0; i < list.length; i++) {
        sampleData.add(new RadioModel(sampleData.length - 1, false,
            MdiIcons.fromString(list[i].icon), list[i].name));
                ...
                child: Row(
                    ...
                    children: <Widget>[
                      ...
                      FlatButton(
                        ...
                        textColor: Color.fromRGBO(114, 103, 239, 1),
                        padding: EdgeInsets.only(
                            left: 5.0, top: 15, right: 5.0, bottom: 15),
                        onPressed: () {
                          _selectTime(context);
                        },
                        child: Column(
                    ...
                    if (_name != null && _surname != null) {
                      if (selected_radio <= 7) {
                        Task now_client = new Task(
                            ...
                            marker:
                                selected_radio == 0 ? 0 : selected_radio - 1,
                            icon: "none",
                          ...
                          check().then((intenet) {
                            if (intenet != null && intenet && registration) {
                              DBUserProvider.dbc
                                  ...
                                  .then((userIdServer) {
                                httpGetWithAllert(
                                    "https://delau.000webhostapp.com/flutter/addTask.php?header=" +
                                        _name +
                                        "&body=" +
                                        _surname +
                                        "&date=" +
                                        ...
                                        "&user_id=" +
                                        userIdServer.toString() +
                                        "&fromMobile=1&mobile_id=" +
                                        id.toString());
                              });
                      ...
                      _firebaseMessaging.getToken().then((token) {
                        httpGet(
                            "https://delau.000webhostapp.com/flutter/addNotif.php?token=" +
                                token +
                                "&date=" +
                                _date.toString() +
                                "&time=" +
